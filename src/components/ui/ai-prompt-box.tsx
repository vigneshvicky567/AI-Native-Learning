            ...
            {files.map((file, index) => (
              <div key={index} className="relative group">
                {file.type.startsWith("image/") && filePreviews[file.name] && filePreviews[file.name] !== "document" ? (
                  <div
                    className="w-16 h-16 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 border border-[#444444]"
                  ...
                  <div className="w-16 h-16 rounded-xl bg-[#2E3033] flex flex-col items-center justify-center p-2 border border-[#444444] relative">
                    <FileText className="h-6 w-6 text-gray-400 mb-1" />
                    <span className="text-[10px] text-gray-300 truncate w-full text-center" title={file.name}>{file.name}</span>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleRemoveFile(index);
